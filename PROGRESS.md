...
- [x] **Binary Ingestion Schema & PDF De-spacing Fixes**: Updated the `UnifiedDocument` schema definition in `backend/ingestion/normalizer.py` to support `Union[str, bytes]` content type, preventing Pydantic `ValidationError` crashes on uploaded files. Implemented a text sanitization heuristic `_clean_pdf_spacing` in `backend/ingestion/document_parser.py` to automatically correct single-character spacings on PDF extractions.
