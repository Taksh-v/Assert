...
class TruthResolver:
    ...
    def resolve_weights(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in results:
            ...
            # Compute new score: prioritize cross_encoder_score, then final_rank_score, then raw score
            original_score = float(
                item.get("cross_encoder_score")
                if item.get("cross_encoder_score") is not None
                else item.get("final_rank_score")
                if item.get("final_rank_score") is not None
            ...
            # Filter low-relevance/low-trust items (lowered threshold to 0.25 to prevent information starvation)
